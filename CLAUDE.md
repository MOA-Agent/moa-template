...
**확인 방법:** 아래 조건 중 하나 이상을 만족하면 이미 초기화된 프로젝트로 판단한다.

- `package.json`의 `dependencies`에 `next` 등 프레임워크가 포함되어 있다
- `app/` 또는 `src/` 디렉토리가 존재한다
- `next.config.js` 또는 `next.config.ts`가 존재한다

- 위 조건을 하나도 만족하지 않으면 → 사용자의 원래 요청을 기억해두고, 먼저 `moa-init` 스킬을 실행한다. Init 완료 후 원래 요청을 이어서 처리한다.
- 하나 이상 만족하면 → 요청에 맞는 스킬을 그대로 실행한다.
