...
2. 프로젝트 폴더명 확인 → `moa-template`이면 영문 프로젝트명 입력받아 폴더 rename
3. L1 질문 (프로젝트 기본 정보) — 4개 질문을 하나씩 순서대로
4. L2 질문 (인프라 선택) — 최대 7개 질문, 조건 분기 포함
5. L3 질문 (스타일) — 1개 질문
6. 선택 조합 요약 → 사용자 확인
7. 코드/설정 파일 생성
8. 문서 파일 생성
9. Vercel 선택 시 인프라 연동 — `references/vercel-setup.md` 참고
...
Git 확인 직후, L1 질문 전에 실행한다.

현재 폴더명이 `moa-template`이면 아래 메시지를 보여주고 새 프로젝트명을 입력받는다.

> "현재 폴더명이 `moa-template`으로 되어 있어요. 새 프로젝트명을 영문으로 입력해주세요. (예: `my-project`, `hub-web`)"

입력받은 이름으로 상위 디렉토리에서 폴더명을 변경한다.

```bash
mv ../moa-template ../{새 프로젝트명}
```

이후 모든 작업은 변경된 폴더 기준으로 진행한다. L1 프로젝트 이름 질문에서는 이 영문명을 기본값으로 제안한다.
