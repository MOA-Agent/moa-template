---
name: moa-init
description: MOA 신규 프로젝트 초기 세팅 스킬. "/init", "init 실행", "프로젝트 세팅 시작", "프로젝트 초기화" 등 프로젝트를 새로 만들거나 초기 환경을 구성하는 요청이 들어오면 반드시 이 스킬을 실행한다. 사용자에게 단계별로 질문해서 프로젝트 기본 정보와 인프라 선택을 수집하고, 그에 맞는 코드/설정 파일과 문서 파일을 자동으로 생성한다. 이미 세팅된 프로젝트(next 등 프레임워크 dependency 존재, 또는 app/·src/ 디렉토리 존재, 또는 next.config.js 존재)에서는 실행하지 않는다.
---
...
1. L1 질문 (프로젝트 기본 정보) — 4개 질문을 하나씩 순서대로
2. L2 질문 (인프라 선택) — 최대 7개 질문, 조건 분기 포함
3. L3 질문 (스타일) — 1개 질문
4. 선택 조합 요약 → 사용자 확인
5. 코드/설정 파일 생성
6. 문서 파일 생성
